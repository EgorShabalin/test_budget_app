...
## Запуск
Для запуска и использования приложения необходимо ввести команду `python3 main.py` и продолжить ввод одной из следующих команд:

#### Для просмотра встроенной справки:

Введите команду `-h` или `--help`
...
Описание: Ужин
```
Далее потребуется указать номер записи для редактирования.
Например `1`:
```
...
Описание: Зарплата
```
Далее потребуется выбрать строку для редактирования.
Напрмер: `3`:
```
...
```
Подтвердите удаление вводом "Да" или "Нет" для отмены удаления записи.
